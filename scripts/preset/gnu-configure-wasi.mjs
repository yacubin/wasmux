  ...
  return {
    ...
    output: {
      ...
      environment: {
        ...
        STRIP: "llvm-strip",
        RANLIB: "llvm-ranlib",
        CFLAGS: `--target=wasm32-wasi -matomics -mmultivalue -mbulk-memory -O3 --sysroot=${path.resolve(process.cwd(), "build/sysroot/usr")}`,
        CXXFLAGS: `--target=wasm32-wasi -matomics -mmultivalue -mbulk-memory -O3 --sysroot=${path.resolve(process.cwd(), "build/sysroot/usr")}`,
